...
public:
  ...
  virtual Json::Value decision(const std::string& action, const Json::Value& json_object);
...
Json::Value 
Myhw5Server::decision(const std::string& action,
 const Json::Value& json_object)
{
  Json::Value result;
  if(json_object.asInt()==1){
    result["decision"]="I'll hit you";
    
  }
  else{
    result["decision"]="I won't hit you";
  }
  return result;
}
...
Myhw5Server::obtain()
{
  ...
  result["done"] = "pull data request recieved";
  this->req = true;
